...
class CartRepoImpl : CartRepo {
    ...
    override fun addToCart(cartItem: CartModel, callback: (Boolean, String) -> Unit) {
        ...
        cartRef.child(id).setValue(cartItem.copy(cartId = id)).addOnCompleteListener {
            if (it.isSuccessful) callback(true, "Added to cart") else callback(false, "${it.exception?.message}")
        }
    }

    override fun getCartItems(userId: String, callback: (Boolean, String, List<CartModel>) -> Unit) {
    ...
    override fun updateQuantity(cartId: String, quantity: Int, callback: (Boolean, String) -> Unit) {
        cartRef.child(cartId).child("quantity").setValue(quantity).addOnCompleteListener {
            if (it.isSuccessful) callback(true, "Updated") else callback(false, "${it.exception?.message}")
        }
    }
    ...
    override fun removeFromCart(cartId: String, callback: (Boolean, String) -> Unit) {
        cartRef.child(cartId).removeValue().addOnCompleteListener {
            if (it.isSuccessful) callback(true, "Removed") else callback(false, "${it.exception?.message}")
        }
    }
